...
# @ registry.cn-hangzhou.aliyuncs.com/kolafim/grape:v2  // 冒号前为镜像名称，后是标签，也可以理解为版本号

docker kill server-test
docker rm server-test
docker run --name server-test -p 8080:8080 -d -e 'NODE_ENV=development' --restart=always registry.cn-hangzhou.aliyuncs.com/kolafim/grape:v2
